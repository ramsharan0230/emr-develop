...
<script type="text/javascript">
    function saveInpatientAllergyDrugs(){
        ...
        var url = "{{route('icu.general.allergydrugstore')}}";
        $.ajax({
            url: url,
            type: "POST",
    ...
    $('.alphabet').on('click', function () {
        ...
        if (ckbox.is(':checked')) {searchdrugs
            ...
            if (chkId.length > 0) {
                $.get("{{'icu.general.searchDrugs'}}", {term: chkId, patient_id: patientid}).done(function (data) {
                    // Display the returned data in browser
                    $("#allergicdrugss").html(data);
                });
            } else {
                $.get("{{ route('icu.general.getAllDrugs') }}", {term: chkId, patient_id: patientid}).done(function (data) {
                    // Display the returned data in browser
                    $("#allergicdrugss").html(data);
                });
            }
    ...
    $("#searchdrugs").keyup(function () {
        ...
        if (searchtext.length > 0) {
            $.get("{{ route('icu.general.searchDrugs') }}", {term: searchtext, patient_id: patientid}).done(function (data) {
                // Display the returned data in browser
                $("#allergicdrugss").html(data)
            });
        } else {
            $.get("{{ route('icu.general.getAllDrugs') }}", {term: searchtext, patient_id: patientid}).done(function (data) {
                // Display the returned data in browser
                $("#allergicdrugss").html(data)
            });
        }
