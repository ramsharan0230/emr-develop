            ...
            <div class="col-sm-12">
                <div class="iq-card">
                    ...
                    <div class="iq-card-body">
                        ...
                        @endif
                        <form action="{{ route('hospital.department.store') }}" method="POST" id="create-hospital-department-form" class="form-horizontal">
                            @csrf

                            <div class="form-group row">
                                <label class="control-label col-sm-2 align-self-center mb-0" for="">Name:</label>
                                <div class="col-sm-4">
                                    <input type="text" id="department_name" name="department_name" class="form-control" value="{{ old('department_name') }}" placeholder="Enter Department Name">
                                </div>
                            </div>
                                ...
                                <div class="col-sm-4">
                                    <select name="parent_department" id="parent_department" class="form-control">
                                        ...
                                        @foreach($parent_departments as $parent_department)
                                            <option value="{{ $parent_department->id }}">{{ $parent_department->name }} ( {{ isset($parent_department->branchData) ? $parent_department->branchData->name : "Main Branch" }} )</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
{{--                                    <input type="text" id="company_id" name="company_id" class="form-control" value="{{ old('company_id') }}" placeholder="Enter Company Id">--}}
                                    <select name="company_id" id="company_id" class="form-control">
                                        ...
                                        @for($i = 1; $i<=200; $i++)
                                            <option value="comp{{ sprintf("%02d", $i) }}">comp{{ sprintf("%02d", $i) }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
