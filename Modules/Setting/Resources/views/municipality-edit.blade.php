            ...
            <div class="iq-card">
                <div class="iq-card-header d-flex justify-content-between">
                    <div class="iq-header-title">
                        <h4 class="card-title">Edit Municipality</h4>
                    </div>
                </div>
                <div class="iq-card-body">
                    ...
                    @endif
                    <form action="{{ route('municipality.update',$municipality_id) }}" method="POST" id="edit-municipality-form" class="form-horizontal">
                        @csrf
                            ...
                            <div class="col-sm-4">
                                <select name="province" id="province" class="form-control">
                                    ...
                                    @foreach($provinces as $province)
                                        <option @if($municipality_data->fldprovince == $province->fldprovince) selected @endif value="{{ $province->fldprovince }}">{{ $province->fldprovince }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="text" id="municipality" name="municipality" class="form-control" value="{{ $municipality_data->fldpality }}" placeholder="Enter Municipality Name">
                            </div>
...
@push('after-script')
    ...
    <script>
        ...
        $(document).ready(function(){
            var districtSelected = "{{ $municipality_data->flddistrict }}";
            getDistrict($('#province').val(), districtSelected);
        });
