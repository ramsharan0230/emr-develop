                                ...
                                <table class="table table-striped table-hover table-bordered">
                                    ...
                                    @if($ledgers)
                                        @foreach($ledgers as $key => $ledger)
                                            @if(!in_array($ledger->AccountNo,$accountIds))
                                                <tr>
                                                    @php
                                                        array_push($accountIds, $ledger->AccountNo);
                                                        $sumTotalCredit = 0;
                                                        $finalCount = 0;
                                                        $ledgersByAccountNumber = $ledgers->where('AccountNo', $ledger->AccountNo)->all();
                                                    @endphp
                                                    @foreach($ledgersByAccountNumber as $key => $ledgerByAccount)
                                                        @php
                                                            if ($ledgerByAccount->accountLedgerDiscount && !in_array($ledgerByAccount->accountLedgerDiscount->AccountNo, $ledgerDiscount)){
$ledgerDiscount[$ledgerByAccount->accountLedgerDiscount->AccountNo] = $ledgerByAccount->accountLedgerDiscount->AccountName;
    }
                                                            $sumTotalCredit += $ledgerByAccount->TranAmount;
                                                        @endphp

                                                        @php
                                                            array_push($tempIdArray, $ledgerByAccount->TranId)
                                                        @endphp
                                                    @endforeach
                                                    <input type="hidden" name="tempId" value="{{ implode("--", $tempIdArray) }}">
                                                    {{--because all transaction have same data except for amount--}}
                                                    <td class="text-center">{{ $countRows++ }}</td>
                                                    <td class="text-center">
                                                        <a href="javascript:;" data-toggle="modal" data-target=".detailed-account-information">{{ $ledger->accountLedger ? $ledger->accountLedger->AccountName : '' }}</a>
                                                    </td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" id="credit-total">{{ $grandTotal }}</td>
                        ...
                        @foreach($ledgers as $key => $ledger)
                            @if($ledger->patbill)
                                <tr>
                                    ...
                                    <td>{{ $ledger->patbill->flditemtype }}</td>
                                    <td>{{ $ledger->patbill->flditemname }}</td>
                                    <td>{{ $ledger->patbill->fldditemamt }}</td>
                                    <td>{{ $ledger->patbill->fldbillingmode }}</td>
