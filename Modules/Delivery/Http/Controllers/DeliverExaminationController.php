...
class DeliverExaminationController extends Controller
{
    /*fixed*/
    public function getExaminationLists(Request $request)
    {
        $flddept = $request->get('flddept') . " Delivery";
    ...
    public function getModalContent(Request $request)
    {
        ...
        if ($type == 'No Selection') {
            ...
            $ret_data = [
                'view_data' => (string) view('delivery::layouts.modal', compact('examid', 'header', 'type')),
            ];
            ...
            $options = \App\DepartmentExamOption::select('fldanswer')
                ...
                ->get();
            $ret_data = [
                'view_data' => (string) view('delivery::layouts.modal', compact('examid', 'header', 'type', 'options')),
                'options' => $options,
            ];
        } else {
            $ret_data = [
                'view_data' => (string) view('delivery::layouts.modal', compact('examid', 'type')),
            ];
        }
    ...
    {
        $encounter_id = \Session::get('delivery_encounter_id');
        $fldinput = $request->get('fldinput') . " Delivery Exam";

        return response()->json(
            PatientExam::select('fldid', 'fldtype', 'fldhead', 'fldabnormal', 'fldtime', 'fldsave', 'fldencounterval', 'fldrepquali', 'fldsysconst')
                ->where([
    ...
    public function savePatientExaminations(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                'message' => __('messages.success', ['name' => 'Examinations']),
            ], 200);
        } catch (Exception $e) {
            return response()->json([
                'status'=> FALSE,
