...
@section('title', 'ITEM PARTICULARWISE REPORT')
...
@section('content')
    ...
    <h2>Item Particular Wise Report</h2>
    ...
    <table style="width: 100%;"  class="content-body">
        ...
        <tbody>
            ...
            @foreach ($datas as $itemtype => $data)
                ...
                @endphp
                <tr>
                    <td colspan="6" style="text-align: center;"><b>{{ $itemtype }}<b></td>
                </tr>
                @foreach($data as $d)
                    @php
                        $total_qty += $d->qnty;
                    ...
                    <tr>
                        <td>{{$d->flditemname}}</td>
                        <td>{{ Helpers::numberFormat($d->rate) }}</td>
                        <td>{{$d->qnty}}</td>
