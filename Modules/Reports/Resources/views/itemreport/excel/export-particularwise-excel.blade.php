...
<table>
    ...
    <tbody>
        ...
        @foreach ($datas as $itemtype => $data)
            ...
            @endphp
            <tr>
                <td colspan="6" style="text-align: center;"><b>{{ htmlspecialchars($itemtype) }}</b></td>
            </tr>
            @foreach($data as $d)
                @php
                    $total_qty += $d->qnty;
                ...
                <tr>
                    ...
                    <td>{{$d->qnty}}</td>
                    <td>Rs. {{  \App\Utils\Helpers::numberFormat(($d->dsc)) }}</td>
                    <td>Rs. {{  \App\Utils\Helpers::numberFormat(($d->tax)) }}</td>
                    <td>Rs. {{  \App\Utils\Helpers::numberFormat(($d->totl + $d->dsc)) }}</td>
                    <td>Rs. {{  \App\Utils\Helpers::numberFormat(($d->totl)) }}</td>
                </tr>
            @endforeach
        @endforeach
    </tbody>
