                ...
                <div class="iq-card iq-card-block iq-card-stretch iq-card-height">
                    <div class="iq-card-header d-flex justify-content-between">
                        <div class="iq-header-title">
                            <h4 class="card-title">Demand vs Order vs Purchase Report</h4>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            <div class="table-responsive table-container">
                                <table class="table table-bordered table-hover table-striped ">
                                    ...
                                    <tbody id="js-ordeer-vs-recieve-order-tbody">
                                    <tr>
                                        <td colspan="14" align="center">Click refresh to generate</td>
                                        {{--                                        <td></td>--}}
                                        {{--                                        <td></td>--}}
                                        {{--                                        <td></td>--}}
                                        {{--                                        <td></td>--}}
    ...
    <script type="text/javascript">
        ...
        $('#refreshBtn').click(function () {
            ...
            if (from_date != '' || to_date != '') {
                $.ajax({
                    url: baseUrl + '/demand-vs-order-vs-receive/getList',
                    type: "GET",
                    data: {
                    ...
                    success: function (response) {
                        ...
                        if (response.status) {
                            $('#js-ordeer-vs-recieve-order-tbody').empty().append(response.html);
                        } else {
                            html += "<tr><td colspan='9' align='center'> No data available<td></tr>";
                            $('#js-ordeer-vs-recieve-order-tbody').empty().append(html);
                        }
                        if (response.error) {
        ...
        $('#exportBtn').click(function () {
            ...
            if (from_date != '' || to_date != '') {
                var url = baseUrl + '/demand-vs-order-vs-receive/report?reference='+reference+'&department=' + department + '&supplier=' + supplier + '&from_date=' + BS2AD(from_date) + '&to_date=' + BS2AD(to_date)
        ...
        $('#excelBtn').click(function () {
            ...
            var reference = $('#reference').val();
            if (from_date != '' || to_date != '') {
                var url = baseUrl + '/demand-vs-order-vs-receive/report/excel?reference='+reference+'&department=' + department + '&supplier=' + supplier + '&from_date=' + BS2AD(from_date) + '&to_date=' + BS2AD(to_date)
                window.open(url, '_blank');
            } else {
                showAlert('Please enter from date and to date', 'error');
            }
        ...
        function getReferences(){
            ...
            if (from_date != '' || to_date != '') {
                $.ajax({
                    url: baseUrl + '/demand-vs-order-vs-receive/getReferences',
                    type: "GET",
                    data: {
                        from_date: BS2AD(from_date),
                        to_date: BS2AD(to_date),
        ...
        function SearchmoreData(page){
            ...
            var from_date = $('#from_date').val();
            var to_date = $('#to_date').val();
            var url = "{{route('demand-vs-order-vs-receive.getList')}}";
            $.ajax({
                url: url+"?page="+page,
                type: "GET",
                data: {
                ...
                success: function(response) {
                    if(response.status){
                        $('#js-ordeer-vs-recieve-order-tbody').html(response.html)
                    }
                },
