            ...
            <div class="iq-card iq-card-block iq-card-stretch iq-card-height">
                <div class="iq-card-header d-flex justify-content-between">
                    <div class="iq-header-title">
                        <h4 class="card-title">
                            Revenue average per department
                        </h4>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    <a href="{{route('getAvgRevenuePerDept')}}" type="button" class="btn btn-light btn-action" ><i class="fa fa-redo"></i>&nbsp;Reset</a>
                                </div>
                            </div>
                        </div>
                    </form>
                        ...
                        <div class="tab-pane fade show active" id="grid" role="tabpanel" aria-labelledby="home-tab-grid">
                            <div class="table-responsive table-sticky-th">
                                ...
                                >
                                    ...
                                    <tbody>
                                    @if(!$avg_rev_dept->isEmpty())
                                        <?php
                                        $count = 1;
                                        ?>
                                        @foreach ($avg_rev_dept as $list)
                                        <tr data-dept-id="{{$list->id}}" class="avg-rev-dept-list">
                                            <td>{{$count++}}</td>
                                            <td>{{$list->name}} </td>
...
<script>
    ...
    $(document).on('click', ".avg-rev-dept-list", function () {
        var dept_id = $(this).data('dept-id');
        window.location.href = baseUrl+"/department/avg-revenue-detail/"+dept_id+'?eng_from_date='+$('#eng_from_date').val()
