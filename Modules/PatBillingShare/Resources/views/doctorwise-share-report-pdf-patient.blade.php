                ...
                @foreach($result as $itemname => $res)
                    ...
                    <tr>
                        ...
                        @if(in_array($itemtype,['OT Dr. Individual','OT Dr. Group','OT Anesthesia','OT  OT Assistant','ICU Procedure']))
                            ...
                            <td></td>
                        @endif
                        @if(in_array($itemtype,['payable','OPD Consultation']))
                        <td>@php
                            $grqtycount += Helpers::getquantitybyitemname($itemtype,$itemname,$doc_name,$eng_from_date,$eng_to_date);
                            @endphp
                            {{ Helpers::getquantitybyitemname($itemtype,$itemname,$doc_name,$eng_from_date,$eng_to_date) }} </td>
                        @else
                        <td>@php
                            $grqtycount += Helpers::getquantitybyitemname($itemtype,$itemname,$doc_name,$eng_from_date,$eng_to_date);
                            @endphp {{ Helpers::getquantitybyitemname($itemtype,$itemname,$doc_name,$eng_from_date,$eng_to_date) }} </td>
                        @endif
                    ...
                    @if(in_array($itemtype,['OPD Consultation']))
                        <tr>
                            <th colspan="10">&nbsp; </th>
{{--                            <th style="text-align: left; font-weight: bold;" colspan="5">{{$itemname}}</th>--}}
                        ...
                        @php
                            $patientdetail = Helpers::getTestedDatewise($itemname,$bill_no,$eng_from_date,$eng_to_date,$flditemname,$doc_name,$itemtype);
                        @endphp
                        @if($patientdetail)
