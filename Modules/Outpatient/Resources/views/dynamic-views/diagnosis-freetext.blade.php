<!-- <form action="{{ route('patient.diagnosis.freetext.save.waiting') }}" id="diagnosis-request-submit" class="laboratory-form container" method="post"> -->
    <!-- @csrf -->
    @php
        $encounterData = $encounter[0];
        $encounterDataPatientInfo = $encounter[0]->patientInfo;
    @endphp
    <input type="hidden" name="encounter" value="{{ $encounterId }}" id="diag_enc">
    <input type="hidden" name="fldinput" value="Provisional Diagnosis" id="diag_fldinput">  
...
<script type="text/javascript">
    $('#submitfreetextdiagnosis').on('click', function(){
        ...
        var encounter = $('#diag_enc').val();
        var fldinput = $('#diag_fldinput').val();
        if(postval !=''){
            var url = "{{ route('patient.diagnosis.freetext.save.waiting') }}";
