...
#account-table tr td:first-child, #account-table tr th:first-child {
    text-align: right;
}

#account-table tr td:nth-child(2), #account-table tr th:nth-child(2) {
    text-align: right;
}
.account {
    text-align: right;
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                <div class="form-group form-row">
                                    ...
                                    <div class="col-sm-6">
                                    @if(!empty($enpatient->flddod))
                                        <input type="text" name="discharge_date" value="<?php echo date('Y-m-d', strtotime($enpatient->flddod)); ?>" class="form-control">
                                    @else
                        ...
                        <table class="table table-striped table-hover table-bordered">
                            ...
                            <tbody class="item-list">
                                ...
                                @foreach($billing['details'] as $bill_detail)
                                    ...
                                    @php
                                        $payables = \App\Utils\BillHelpers::getBillPayables($bill_detail->fldtempbillno);
                                    @endphp

                                    @if(isset($bill_detail) && ($bill_detail->serviceCost->discount == '0' or !is_null($bill_detail->noDiscount)))
                                        @php
                                            $discountFlag = 0;
                                    ...
                                    @else
                                        @php
                                            $alloweddiscountamount += $bill_detail->flditemrate * $bill_detail->flditemqty;
                                            $discountFlag = 1;
                                        @endphp
                                    @endif
                                <tr data-tax="{{$bill_detail->fldtaxamt}}" data-discount="{{$discountFlag}}">
                                    <td>{{$bill_detail->serviceCost->fldbillitem}} @if($discountFlag == 1)*@endif</td>
...
{{--                                    <td>{!! Helpers::getNameByUsername($bill_detail->fldpayto)!!}</td>--}}
            ...
            @endphp
            <input type="hidden" name="allowed_discount_amount" id="allowed_discount_amount" value="{{$alloweddiscountamount}}">
            ...
            <div class="iq-card iq-card-block iq-card-stretch iq-card-height">
                ...
                <div class="iq-card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered " id="account-table">
                            <thead class="thead-light">
                                <tr>
                                    <th>Particulars</th>
                            ...
                            <tbody>
                                ...
                                @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="text" name="category_total" class="form-control" readonly value="{{ \App\Utils\Helpers::numberFormat($category[$billing['category']])}}"></td>
                                    </tr>
                                    @php
                                ...
                                <tr>
                                    <th class="text-bold">Sub Total</th>
                                    <th>@if(isset($total)) {{ \App\Utils\Helpers::numberFormat($subtotal) }} @endif</th>
                                    <input type="hidden"  class="subtotal" id="" value="@if(isset($total)) {{ $subtotal }} @endif">
                                </tr>
                                <tr>
                                    <th class="text-bold">Discount Total</th>
                                    <th id="displayDiscountTotal">@if(isset($discount)) {{ \App\Utils\Helpers::numberFormat($discount) }} @endif
                                ...
                                <tr>
                                    ...
                                    <th id="totalAmt">@if(isset($total)) {{ \App\Utils\Helpers::numberFormat(($subtotal - $discount)) }} @endif</th>
                                </tr>
                                <tr>
                                    <td>Tax</td>
                                    <td><input type="text" class="form-control account" id="tax" value="{{ \App\Utils\Helpers::numberFormat($tax) }}" readonly></td>
                                </tr>
                                <!--                                <tr>
                                                                    <td class="">Total Discount</td>
                                                                    <td class=""><input type="text" class="form-control" id="totaldiscount"></td>
                                                                </tr>-->
                                <tr>
                                    <th class="text-bold">Net Total</th>
                                    <th id="nettotal">@if(isset($grandTotal)) {{ \App\Utils\Helpers::numberFormat($grandTotal) }} @endif</th>
                                    <input type="hidden"  class="nettotal" id="" value="@if(isset($grandTotal)) {{ $grandTotal }} @endif">
                                </tr>

                                <tr>
                                    <th class="text-bold "> Total Deposit</th>
                                    <th id="curdeposit">@if(isset($previousDeposit)){{ \App\Utils\Helpers::numberFormat($previousDeposit) }} @endif</th>
                                </tr>
                                <?php if (isset($total)) {
                                    $tobepaidhospital = 0;
                                    $tobepaidpatient = 0;
                                ...
                                <tr>
                                    ...
                                    <td ><input type="text" class="form-control account" id="remaining_deposit" value="@if(isset($remaining_deposit)) {{\App\Utils\Helpers::numberFormat($remaining_deposit->fldcurdeposit)}}  @endif" readonly></td>
                                </tr>
                                <tr>
                                    <td >To be Paid</td>
                                    <td ><input type="text" class="form-control account" id="tobepaid" value="@if(isset($tobepaid)) {{\App\Utils\Helpers::numberFormat($tobepaid)}}  @endif" readonly></td>
                                </tr>
                                <tr>
                                    <td >To Refund</td>
                                    <td ><input type="text" class="form-control account" id="tobepaidbyhospital" value="{{\App\Utils\Helpers::numberFormat($tobepaidhospital)}}" readonly></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td >To be Paid by Patient</td>
                                    <td ><input type="text" class="form-control account" id="tobepaidbypatient" value="{{\App\Utils\Helpers::numberFormat($tobepaidpatient)}}" readonly></td>
                                </tr>
                                @if(isset($remaining_deposit->fldcurdeposit) and ($remaining_deposit->fldcurdeposit > $grandTotalFirst))
                                    @php
                                        $disable = 'disabled';
                                    @endphp
                                @else
                                ...
                                <tr>
                                    ...
                                    <td ><input type="text" class="form-control account" id="received_amount" value="{{\App\Utils\Helpers::numberFormat($recieved_amount)}}" {{$disable}}></td>
                                </tr>

                                <tr>
                                    <td >CUR Deposit</td>
                                    <td ><input type="text" class="form-control account" id="cur_deposit" value="{{\App\Utils\Helpers::numberFormat($cur_deposit)}}"></td>
                                </tr>
                                ...
                                <tr>
                                    <td class="">Remarks</td>
                                    <td class=""><textarea id="discharge_remark" class="form-control"></textarea></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    @if(isset($remaining_deposit->fldcurdeposit) and ($remaining_deposit->fldcurdeposit > $grandTotalFirst))
                        <input type="hidden" name="mode" value="0" class="mode">
                    @else
                    <label><b> Payment Mode:</b></label>
                        <div class="bak-payment p-2">
                            <div class="form-row">
                                ...
                                <div class="col-sm-3  pay-rad checked-bak" id="cash_payment" onclick="getRadioFunction('Cash')">
                        ...
                        <input type="hidden" name="mode" value="1" class="mode">
                    ...
                    <div class=" form-group form-row">
                        ...
                        <div class="col-sm-12 text-right">
                            ...
                            <a href="javascript:;" class="btn btn-primary btn-action" onclick="previewDischargeClearance()"><i class="fa fa-eye"></i>&nbsp;Preview</a>

                            <a href="javascript:;" class="btn btn-primary btn-action" onclick="printDischargeClearance()"><i class="fa fa-print"></i>&nbsp;Print</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function getRadioFunction(value){
            if (value == "Cash") {
                ...
                deposit = parseFloat(numberFormat($('#curdeposit').text()));
                if(parseInt(deposit) > parseInt(net)){

                }else{
                    $('#received_amount').val(numberFormatDisplay(parseFloat(net)-parseFloat(deposit)));
                    $('#tobepaidbyhospital').val(numberFormatDisplay(0));
                    $('#tobepaidbypatient').val(numberFormatDisplay(parseFloat(net)-parseFloat(deposit)));
                ...
                $('#cur_deposit').val(numberFormatDisplay(parseFloat(deposit) - parseFloat(net)));
        ...
        function fonepayQrGenerate(encounter) {
            let route = "{!! route('convergent.payments.dischargeClearance') !!}";
            $.ajax({
                url: route,
                type: "POST",
        ...
        $(document).on('change', '#discount_type_change', function () {
            ...
            if (discount_type == 'no_discount') {
                $('#discountAmount').val(numberFormatDisplay(0));
                $('#discountPercentange').val(numberFormatDisplay(0));
        ...
        $(document).on('focusout', '#discountAmount', function () {
            var netamount = parseFloat(numberFormat($('.nettotal').val()));
            var discamt = parseFloat(numberFormat($(this).val()));
            var alloweddiscount = parseFloat(numberFormat($('#allowed_discount_amount').val()));
            if(parseFloat(discamt) > parseFloat(alloweddiscount)){
                $('#discountAmount').val('');
                showAlert('Discount cannot be greater than '+alloweddiscount, 'fail');
                return false;
            }
            if(parseFloat(discamt) > parseFloat(netamount)){
                $('#discountAmount').val('');
            ...
            var percentage = (discount*100)/alloweddiscount;
    ...
    $(function() {
        ...
        $('#received_amount').on('keyup', function() {
            ...
            // alert(tobepaidhospital);
            // alert(tobepaidpatient);
        ...
        $('#totaldiscount').on('keyup', function() {
            ...
            $('#tobepaid').val(numberFormatDisplay(parseFloat(nettotal) - (parseFloat(nettotal) + parseFloat(dis))))
        });
        ...
        $('#discountPercentange').keyup(function () {
                ...
                if (isNaN(discount)) {
                    ...
                    showAlert('Discount cannot be greater than 100 %', 'fail');
                    $('#discountAmount').val('');
                    $('#discountPercentange').val(numberFormat(0));
        ...
        $('.payment-refund').on('click', function() {
            ...
            if (cur_deposit > 0) {
                if (confirm('Do you want to refund deposit?') == true) {
                    $.ajax({
                        url: "{{ route('billing.finalPaymentDischargeRefundDeposit') }}",
                        type: "POST",
                        data: {
    ...
    function printDischargeClearance() {
        ...
        var queryString = $.param(params);
        window.open("{{ route('discharge.clearance.print') }}?" + queryString, '_blank');
    }

    function previewDischargeClearance() {
        var params = {
            encounter_id: $('#encounter_id').val(),
            print_preview: $('input[name=print_preview]:checked').val(),
            billtype:'discharge'
        ...
        var queryString = $.param(params);
        window.open("{{ route('discharge.preview.invoice') }}?" + queryString, '_blank');
    }
</script>
<script>
...
<script>
    var params = {
        ...
        print_preview: "{{Session::get('print_preview')}}",
        billtype:'discharge'
    };
    var queryString = $.param(params);
