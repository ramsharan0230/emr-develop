...
class ReferalDoctorWiseExport implements FromView,WithDrawings,ShouldAutoSize
{
...
//        dd($data);
        ...
        $data['results'] = PatBillingShare::where('pat_billing_shares.user_id', $doctor_id)
            ...
            ->when(isset($eng_from_date), function ($q) use ($eng_from_date) {
                $data['eng_from_date'] = $eng_from_date . ' 00:00:00';
                return $q->whereDate('tblpatbilling.fldordtime', '>=', $eng_from_date . ' 00:00:00');
            })
            ->when(!(isset($eng_from_date)), function ($q) use ($eng_from_date) {
                $data['eng_from_date'] = date('Y-m-d') . ' 00:00:00';
                return $q->whereDate('tblpatbilling.fldordtime', '>=', date('Y-m-d') . ' 00:00:00');
            })
            ->when(isset($eng_to_date), function ($q) use ($eng_to_date) {
                $data['eng_to_date'] = $eng_to_date . " 23:59:59";
                return $q->whereDate('tblpatbilling.fldordtime', '<=', $eng_to_date . " 23:59:59");
            })
            ->when(!(isset($eng_to_date)), function ($q) use ($eng_to_date) {
                $data['eng_to_date'] = date('Y-m-d') . ' 23:59:59';
                return $q->whereDate('tblpatbilling.fldordtime', '<=', date('Y-m-d') . ' 23:59:59');
            })
        ...
        return view('patbillingshare::excell.doctor-wise-with-referal-excel',$data);
    }
