...
class ReconciliationReportExport implements FromView,WithDrawings,ShouldAutoSize
{
    ...
    public function view(): View
    {
        ...
        $datesql = "SELECT DISTINCT DATE_FORMAT(fldtime,'%Y-%m-%d') AS date FROM tblpatbilldetail where fldtime >= '".$filterdata['eng_from_date']."' and fldtime <= '".$filterdata['eng_to_date']."'";
        $datedata = \DB::select($datesql);
        $department = $filterdata['department'];
        return view('reports::excel.reconciliation-report-excel', compact('nepalifromdate','nepalitodate','filterdata','datedata','department'));
    }
