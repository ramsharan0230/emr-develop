...
class StockTransferExport implements  FromView,WithDrawings,ShouldAutoSize
{
    public function __construct(string $from_date,string $to_date,string $reference)
    {
    ...
    public function view(): View
    {
        ...
        $data['references'] = Transfer::with('entry','brand','extraBrand','surgicalBrand')->where('fldreference',$reference)
            ->whereDate('fldfromentrytime', '>=', $from_date)->whereDate('fldfromentrytime', '<=', $to_date)
//                ->where('fldsave','=','1')
            ->get();
        return view('reports::stock-transfer.stock-transfer-excel',$data);
    }
