...
class Nepalicalendar {
		...
		public function eng_to_nep($yy,$mm,$dd){
			if ($this->is_range_eng($yy, $mm, $dd) == false){
				...
				$this->nep_date['full_date']    = $y . '-' . str_pad($m, 2, 0, STR_PAD_LEFT) . '-' . str_pad($total_nDays, 2, 0, STR_PAD_LEFT);
		...
		public function nep_to_eng($yy,$mm,$dd){
			...
			if($this->is_range_nep($yy, $mm, $dd)===false){
				...
				$this->eng_date['full_date']    = $y . '-' . str_pad($m, 2, 0, STR_PAD_LEFT) . '-' . str_pad($total_eDays, 2, 0, STR_PAD_LEFT);
